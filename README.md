...
| **斗鱼弹幕** `douyu-danmaku-server` | 进程环境变量：`FMZ_TRIGGER_AI_MODEL`、`AI_AGENT_INTERNAL_URL` 或 `AI_AGENT_PORT`；日报分段：`FMZ_AI_REPORT_CHUNK_DM_LINES`、`FMZ_AI_REPORT_CHUNK_FORCE_CHARS`、`FMZ_AI_REPORT_VERBATIM_DM_SAMPLE` | 生产写在 systemd `Environment=` 或宿主 export | 调用内网 `/chat` 须遵守模型降级链（见 Cursor rule）；说明亦在 `local-ai-agent.env.example` 底部 |
