...
| **斗鱼弹幕** `douyu-danmaku-server` | `FMZ_TRIGGER_AI_MODEL`、`AI_AGENT_INTERNAL_URL` / `AI_AGENT_PORT`；**日报体积**：`FMZ_AI_REPORT_MAX_GIFT_LINES`（默认 650）、`FMZ_AI_REPORT_VERBATIM_DM_SAMPLE`（分段时原文抽样行数，默认 160）、`FMZ_AI_REPORT_DAILY_FORCE_CHUNK_CHARS`、`FMZ_AI_REPORT_DAILY_FORCE_MIN_PARTS`、`FMZ_AI_REPORT_CHUNK_DM_LINES` 等 | 生产可用 **`deploy/fmz-danmaku.env.example`** → 远端 `/opt/fmz-danmaku-server/danmaku.env` + systemd `EnvironmentFile=`（见 `RELEASE_AND_DEPLOY.md` §13.2） | 见 `douyu-danmaku-server.mjs` 顶部常量；`local-ai-agent.env.example` 仅示例 AI 网关变量 |
