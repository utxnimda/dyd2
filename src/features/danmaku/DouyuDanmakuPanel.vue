...
/** giftInfo.from：2＝背包（总开销不按 cost 累计）；直连用 CDN v3。**from=2** 时 cost/value 优先由 prop_gift_config(pc/devote) 覆盖，仍缺省则用 v3 刻度。propRaw：道具表本条快照（debug）。 */
interface GiftInfo {
  name: string;
  icon: string;
  cost: number;
...
/** from：1＝CDN 直接礼物（总「花」计 cost÷10）；2＝背包礼（不计入总花，刻度见 JSON）。 */
const GIFT_FROM_HINT =
  "礼物礼单「from」：1＝直接礼物——表中 cost 可按鱼翅刻度÷10折算「花」；2＝背包礼物——本平台总「花」不累计该项 cost（避免误把背包刻度当成现金鱼翅），cost/value 为 CDN 原始刻度，若以其它字段或接口为准请以「打开」内 JSON 核对。";

const GIFT_STATS_TOTAL_COST_HINT =
  "总开销仅 Σ(from≠2 的礼物 cost÷10×件)。from=2 背包礼不包含在内；其价值仍可在「总价值」与各行的「值」里体现。";

const GIFT_CATALOG_VALUE_HINT =
  "服务端拉「gift/v3/web/list」，对 **from=2** 再按斗鱼 **prop_gift_config**（webconf JSONP）覆写 pc→cost、devote→value。**from**—1＝直连礼、2＝背包。**直连：**cost＝单次鱼翅刻度人民币×10；value＝价值人民币元。**背包：**命中 prop 后以 **pc／devote** 为准（贡献向刻度）；未命中则用 v3 原字段。**总花**仅累计 from≠2。";

const GIFT_PROP_TABLE_HINT =
  "斗鱼 webconf 「prop_gift_config」静态 JSON（JSONP）；与站内背包礼配置对齐。表中为**本房间** CDN from=2 条目；斗鱼全库键数量见脚注。**命中配置**：prop 表中存在该 gfid；此时服务端已写入 giftInfo.cost/giftInfo.value。";

const GIFT_PROP_OVERLAY_HINT =
  "是否在全站 prop 表中查到该 gfid：**是** 表示用 pc／devote 校准；**否** 表示仅 CDN v3 price／growth 刻度（可能不适合背包等价）。";

const GIFT_STATS_BY_GIFT_VISIBLE = "计价：直连计花(cost÷10)、背包不计花；值均计 value；命名优先 gfn";

const GIFT_STATS_BY_GIFT_HINT =
  "「花」行仅 from=1 时显示(cost÷10×件)；from=2 仅标「背包」不写花。「值」= value×件。名称优先最晚同 gfid 的 gfn。";
...
/** 礼单列摘要（礼单价）；直连按花/值展示，背包单列刻度与占位说明 */
function giftCatalogUnitDisplay(gfid: string): string {
  const gid = String(gfid ?? "").trim();
  const inf = giftInfoMap.value[gid];
  if (!inf) return "无礼表";
  const from = giftInfoFromNorm(inf);
  const c = inf.cost ?? 0;
  const v = inf.value ?? 0;

  if (from === 2) {
    const segs: string[] = [];
    if (v) segs.push(`devote${giftFmtMoneyYuan(v)}`);
    if (c) segs.push(`pc${giftFmtMoneyYuan(c)}`);
    if (segs.length === 0) return "背包·无价";
    return `背包｜${segs.join("/")}`;
  }

  if (!c && !v) return "无价";
  const segs: string[] = [];
  if (c) segs.push(`花${giftFmtMoneyYuan(c / 10)}元`);
  if (v) segs.push(`值${giftFmtMoneyYuan(v)}元`);
  return segs.join("/");
}
...
function openGiftCatalogRawWindow(gfid: string): void {
  ...
  if (raw != null && typeof raw === "object" && !Array.isArray(raw)) {
    ...
    payload = {
      ...
      _note: "本条无服务端附带的 raw（多为内置兜底或未返回原生片段）；此为当前 giftInfoMap 可见字段快照。from：1直连／2背包；propRaw：命中 prop_gift_config 时的道具表快照。",
    };
...
// 「花」合计(元)：仅 from=1 直连礼 Σ(cost÷10×件)，背包不计入总花（见悬浮说明）
const giftStatsTotalCost = computed(() => {
  ...
  for (const [gfid, v] of Object.entries(giftStats.value.byGift)) {
    ...
    if (giftInfoFromNorm(inf) === 2) continue;
              ...
              <div class="dm-danmaku-stack dm-danmaku-stack--fixed-h">
                ...
                <div v-for="(msg, idx) in filteredDanmakuMessages" :key="idx" class="dm-msg" :class="{ 'dm-msg--cmd': String(msg.txt || '').startsWith('#') }">
                  <div class="dm-msg-chatline">
                    ...
                    >
                      <img
                        v-if="lvlPortraitShow(msg)"
                        :key="lvlPortraitRoomKey(msg) + ':' + lvlPortraitSrc(msg)"
                        :src="lvlPortraitSrc(msg)"
                        class="dm-chat-ulv-portrait"
                        alt=""
                        referrerpolicy="no-referrer"
                        decoding="async"
                        @error="onLvlPortraitErr(msg)"
                      />
                      <span class="dm-chat-pill-ulv-num">{{ msg.level }}</span>
                    </span>
                    <span
                      v-if="msg.bnn"
                      class="dm-chat-pill dm-fan-el"
                      :class="fanMedalQualityClass(msg.bl)"
                    >
                      <span class="dm-fan-seg dm-fan-seg--lv">
                        <span class="dm-fan-lv-inner">
                          <img
                            v-if="badgeAvatar(danmakuFanBrid(msg))"
                            :src="badgeAvatar(danmakuFanBrid(msg))"
                            class="dm-fan-seg-avatar"
                            alt=""
                            referrerpolicy="no-referrer"
                          />
                    ...
                    <span class="dm-chat-ident">
                      <span class="dm-chat-nick">{{ msg.nn }}</span>
                    </span>
                      ...
                      <div class="dm-toolbar-row-first">
                        <div class="dm-toolbar-menu-lead dm-toolbar-menu-lead--gift">
                          <nav class="dm-gift-tabs dm-gift-tabs--inline dm-gift-tabs--toolbar">
                            ...
                            <button type="button" class="dm-gift-tab-debug" :class="{ active: giftSubTab === 'debug' }" title="下行原始字段与计件明细（调试）" @click="giftSubTab = 'debug'">调试</button>
                          </nav>
                        </div>
                        <div class="dm-toolbar-mode-cluster">
                          <div class="dm-toolbar-mode-cluster-actions">
                ...
                <div class="dm-gift-body">
                  ...
                  <div v-if="giftSubTab === 'records'" ref="giftFeedRef" class="dm-gift-feed">
                    ...
                    <div v-for="(g, idx) in giftFilteredList" :key="idx" class="dm-gift-item" :class="{ 'dm-gift--big': g.bg === '1' }">
                      ...
                      >
                        <img
                          v-if="giftLvlPortraitShow(g)"
                          :key="giftLvlPortraitRoomKey(g) + ':' + giftLvlPortraitSrc(g)"
                          :src="giftLvlPortraitSrc(g)"
                          class="dm-chat-ulv-portrait"
                          alt=""
                          referrerpolicy="no-referrer"
                          decoding="async"
                          @error="onGiftLvlPortraitErr(g)"
                        />
                      ...
                      >
                        <span class="dm-fan-seg dm-fan-seg--lv">
                          <span class="dm-fan-lv-inner">
                            <img
                              v-if="badgeAvatar(g.brid || '')"
                              :src="badgeAvatar(g.brid || '')"
                              class="dm-fan-seg-avatar"
                              alt=""
                              referrerpolicy="no-referrer"
                            />
                      ...
                      <span v-if="giftRecordGfidLabel(g)" class="dm-gift-gfid-hint">{{ giftRecordGfidLabel(g) }}</span>
                    </div>
                  </div>
                  <!-- Stats view -->
                  <div v-if="giftSubTab === 'stats'" class="dm-gift-stats">
                    ...
                    <template v-else-if="giftStats">
                      <div class="dm-gift-stats-summary">
                        ...
                        <div class="dm-gift-stats-card" :title="GIFT_CATALOG_VALUE_HINT">
                      ...
                      <div class="dm-gift-stats-section">
                        ...
                        <p class="dm-gift-stats-source-hint" :title="GIFT_STATS_BY_GIFT_HINT">{{ GIFT_STATS_BY_GIFT_VISIBLE }}</p>
                        <div v-for="item in giftStatsByGiftSorted" :key="item.gfid" class="dm-gift-stats-row">
                          ...
                          <span class="dm-gift-stats-name"
                            >{{ item.name }} <span class="dm-gift-stats-gfid">#{{ item.gfid }}</span></span
                          >
                          <span class="dm-gift-stats-cnt">×{{ item.count }}</span>
                          <span v-if="item.value" class="dm-gift-stats-price">值{{ (item.value * item.count).toFixed(1) }}元</span>
                          <span v-if="item.from === 2" class="dm-gift-stats-from-badge" :title="GIFT_FROM_HINT">背包</span>
                          <span v-if="item.from !== 2 && item.cost" class="dm-gift-stats-cost">花{{ (item.cost * item.count / 10).toFixed(1) }}元</span>
                        </div>
                  ...
                  <div v-if="giftSubTab === 'debug'" class="dm-gift-feed dm-gift-debug-feed">
                    ...
                    <template v-else>
                      <p class="dm-gift-debug-lead">
                        ...
                        hits、gs。下列为当前<strong>筛选</strong>结果，自上而下为<strong>新 → 旧</strong>。<strong>gfid</strong>＝斗鱼礼物 id；<strong>gfn</strong>＝本条下行名称；可与下方「查找表」中礼单名称对照。<strong>礼单价(花／值)</strong>按 gfid 查当前房间礼单缓存，列表头悬停见详解。下行<strong>完整 JSON</strong>仅在新窗口<strong>打开</strong>查看。
                      </p>
                      <details class="dm-gift-debug-lookup">
                        <summary class="dm-gift-debug-lookup-sum">礼物信息查找表（gfid → 开销刻度×10／价值·元）</summary>
                        <p class="dm-gift-debug-lookup-note" :title="GIFT_CATALOG_VALUE_HINT">
                          ...
                          <strong>开销×10</strong>（giftInfo.cost，人民币×10）与<strong>价值(元)</strong>（giftInfo.value，人民币元）及<strong>礼单价摘要</strong>同源（列表头悬停）。末列<strong>打开</strong>在新窗口查看该 gfid 的 CDN 本条 JSON（无 <code class="dm-gift-debug-code">raw</code> 时为映射快照）。
                        </p>
                            ...
                            <table class="dm-gift-debug-catalog-table">
                              <thead>
                                <tr>
                                  ...
                                  <th>礼单名</th>
                                  <th title="giftInfo.cost：单次礼物开销，人民币×10（折合÷10为元）">开销×10</th>
                        ...
                        <table class="dm-gift-debug-table">
                          <thead>
                            <tr>
                              ...
                              <th :title="GIFT_CATALOG_VALUE_HINT">礼单价(花／值)</th>
                              <th>昵称</th>
                              <th>uid</th>
                              <th>bg</th>
                              <th>rid</th>
                  ...
                  <div v-for="(msg, idx) in filteredDanmakuMessages" :key="'df_' + idx" class="dm-msg" :class="{ 'dm-msg--cmd': String(msg.txt || '').startsWith('#') }">
                    <div class="dm-msg-chatline">
                      ...
                      <span v-if="msg.level" class="dm-chat-pill dm-ulv-el" :class="userLevelQualityClass(msg.level)">
                        <img
                          v-if="lvlPortraitShow(msg)"
                          :key="'dfl:' + lvlPortraitRoomKey(msg) + ':' + lvlPortraitSrc(msg)"
                          :src="lvlPortraitSrc(msg)"
                          class="dm-chat-ulv-portrait"
                          alt=""
                          referrerpolicy="no-referrer"
                      ...
                      <span v-if="msg.bnn" class="dm-chat-pill dm-fan-el" :class="fanMedalQualityClass(msg.bl)">
                        <span class="dm-fan-seg dm-fan-seg--nm">
                          <span class="dm-chat-pill-bnn">{{ msg.bnn }}</span>
                        </span>
                      </span>
                  ...
                  <div v-if="giftSubTab === 'stats'" class="dm-gift-stats">
                    ...
                    <template v-else-if="giftStats">
                      <div class="dm-gift-stats-summary dm-gift-stats-summary--compact">
                        ...
                        <span :title="GIFT_STATS_TOTAL_COST_HINT">花<strong>{{ giftStatsTotalCost.toFixed(1) }}</strong>元</span>
                      </div>
                      <p class="dm-gift-stats-source-hint dm-gift-stats-source-hint--compact" :title="GIFT_STATS_BY_GIFT_HINT">
                        {{ GIFT_STATS_BY_GIFT_VISIBLE }}
                      </p>
                      <div v-if="giftStatsByGiftSorted.length === 0" class="dm-empty">暂无数据</div>
                      <div v-for="item in giftStatsByGiftSorted" :key="'gfs_' + item.gfid" class="dm-gift-stats-row">
                        <span class="dm-gift-stats-name"
                          >{{ item.name }} <span class="dm-gift-stats-gfid">#{{ item.gfid }}</span></span
                        >
                        <span class="dm-gift-stats-cnt">×{{ item.count }}</span>
              ...
              <div v-for="(msg, idx) in filteredDanmakuMessages" :key="'pop-' + idx" class="dm-msg" :class="{ 'dm-msg--cmd': String(msg.txt || '').startsWith('#') }">
                <div class="dm-msg-chatline">
                  ...
                  >
                    <img
                      v-if="lvlPortraitShow(msg)"
                      :key="'popimg:' + lvlPortraitRoomKey(msg) + ':' + lvlPortraitSrc(msg)"
                      :src="lvlPortraitSrc(msg)"
                      class="dm-chat-ulv-portrait"
                      alt=""
                      referrerpolicy="no-referrer"
                ...
                <div v-if="giftSubTab === 'records'" ref="giftFeedPopRef" class="dm-gift-feed">
                  ...
                  <div v-for="(g, idx) in giftFilteredList" :key="'gift-pop-' + idx" class="dm-gift-item" :class="{ 'dm-gift--big': g.bg === '1' }">
                    ...
                    >
                      <img
                        v-if="giftLvlPortraitShow(g)"
                        :key="'gift-poplvl:' + giftLvlPortraitRoomKey(g) + ':' + giftLvlPortraitSrc(g)"
                        :src="giftLvlPortraitSrc(g)"
                        class="dm-chat-ulv-portrait"
                        alt=""
                        referrerpolicy="no-referrer"
                ...
                <div v-if="giftSubTab === 'stats'" class="dm-gift-stats">
                  ...
                  <template v-else-if="giftStats">
                    ...
                    <div class="dm-gift-stats-section">
                      <div class="dm-gift-stats-section-title">按礼物</div>
                      <p class="dm-gift-stats-source-hint" :title="GIFT_STATS_BY_GIFT_HINT">{{ GIFT_STATS_BY_GIFT_VISIBLE }}</p>
                      <div v-for="item in giftStatsByGiftSorted" :key="'gift-pop-stats-g-' + item.gfid" class="dm-gift-stats-row">
                        <img v-if="item.icon" :src="item.icon" class="dm-gift-icon-sm" alt="" referrerpolicy="no-referrer" />
                        <span v-else class="dm-gift-icon-sm-placeholder">🎁</span>
                        <span class="dm-gift-stats-name"
                          >{{ item.name }} <span class="dm-gift-stats-gfid">#{{ item.gfid }}</span></span
                    ...
                    <div class="dm-gift-stats-section">
                      <div class="dm-gift-stats-section-title">按用户</div>
                      <div v-for="item in giftStatsByUserSorted" :key="'gift-pop-stats-u-' + item.uid" class="dm-gift-stats-row">
                        <span class="dm-gift-stats-nick">{{ item.nn || item.uid }}</span>
                        <span class="dm-gift-stats-cnt">×{{ item.count }}</span>
                      </div>
                      <div v-if="giftStatsByUserSorted.length === 0" class="dm-empty">暂无数据</div>
...
.dm-ulv-el.dm-ulv-dy0 .dm-chat-ulv-portrait {
...
/* 弹幕粉丝牌固定总宽：左段≈三位数字+小头像槽，右段≈三字牌名槽，分界线位置不变 */
...
.dm-chat-pill-lv { font-weight: 700; opacity: 0.95; }
